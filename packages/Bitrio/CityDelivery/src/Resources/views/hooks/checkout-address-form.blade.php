@pushOnce('scripts')
    ...
    <script type="module">
        app.component('v-checkout-address-form', {
            ...
            mounted() {
                console.log('v-checkout-address-form: Componente montado. Iniciando carga de datos.');
                this.getCountries();
                this.loadCities();
            },
            ...
            methods: {
                ...
                loadCities() {
                    debugger
                    this.$axios.get('/citydelivery/cities')
                        .then(res => {
                ...
                updateDeliveryCost() {
                    ...
                    if (!this.address.city_id) return;

                    this.$axios.post('/citydelivery/rate', {
                        city_id: this.address.city_id,
                    })
                    .then(res => {
                        // Actualiza el carrito
                        this.$emitter.emit('cart-updated', res.data);
                    })
                    .catch(err => {
